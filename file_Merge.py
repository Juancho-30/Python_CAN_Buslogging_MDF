...
path_list = ['C:/Users/'] #Introduce the path where your mdf files to be merged are located

start = input("Introduce the start time in 2 digit format as the following HH:MM ") # If you want to cut the merged file according to the relative start time

end = input("Introduce the end time in 2 digit format as the following HH:MM ") # If you want to cut the merged file according to the relative stop time

date = input("Introduce the date together as YYYYMMDD ") # Used as reference to go through the path in path list and save with the same date
